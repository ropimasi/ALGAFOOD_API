...
public class EstadoCadastroService {
	...
	public Estado salvar(Estado estado) {
		...
		try {
			return estadoRepo.gravar(estado);
	...
	public Estado excluir(Long id) {
		 ...
		 * Tais como condições obrigatórias, validações. */
		try {
			return estadoRepo.remover(id);
		} catch (EmptyResultDataAccessException excep) {
			throw new EntidadeNaoEncontradaException(
					String.format("Estado 'id=%d' não pode ser removido, pois não existe. ", id));
		} catch (DataIntegrityViolationException excep) {
			throw new EntidadeEmUsoException(
					String.format("Estado 'id=%d' não pode ser removido, pois está em uso. ", id));
		} /* catch (Exception excep) {
			 * throw new RuntimeException(
			 * String.format("Erro INESPERADO na exclusão Estado 'id=%d'. ", id));
			 * } */
